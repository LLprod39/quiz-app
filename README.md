...
Организатор может выбрать набор в разделе «Каталог квизов». Установка создаёт новое мероприятие и переносит прежнее активное мероприятие в архив только после подтверждения; история завершённых игр сохраняется. Вопросы пакетов хранятся локально, поэтому их можно запускать без обращения к внешнему API.
