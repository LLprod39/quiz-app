...
import { Activity, Archive, ArrowLeft, ArrowRight, BarChart3, BookOpenText, Check, ChevronRight, CircleHelp, ClipboardList, Copy, Crown, ExternalLink, Gamepad2, Gauge, Headphones, History, LayoutDashboard, Library, Link2, LoaderCircle, LogOut, Monitor, MoreVertical, PartyPopper, Pencil, Play, Plus, QrCode, Radio, RotateCcw, Save, Send, Settings2, Share2, ShieldCheck, Smartphone, Sparkles, Trash2, Upload, Users, Wifi } from 'lucide-react'
import { Link } from '../lib/router'
import { QRCodeSVG } from 'qrcode.react'
import { api, ApiError } from '../lib/api'
import { createId } from '../lib/id'
import { useRememberedNavigationState } from '../lib/navigationState'
import { cleanQuizPackJsonInput } from '../lib/quizPackImport'
import { DEFAULT_BRANDING, THEME_PRESETS, themeStyle, useBranding } from '../lib/branding'
import { useGameStore } from '../store/game'
import type { Account, ContentMode, EventData, Question, QuestionnaireItem, QuizPack, Snapshot, ThemeConfig } from '../types'
import { Badge, Button, Card, ConnectionPill, Empty, Field, Logo, SaveState, formatTime } from '../components/ui'
import { Timer } from '../components/Timer'
import { QuestionSpeechEditor } from '../components/QuestionSpeechEditor'
import { QuizPackCard } from './QuizCatalogPage'
import { SystemAdminPanel } from './SystemAdminPage'

type Tab = 'overview' | 'quizzes' | 'catalog' | 'settings' | 'questionnaire' | 'editor' | 'rehearsal' | 'share' | 'live' | 'history' | 'system'
const rememberedTabs: readonly Tab[] = ['overview', 'quizzes', 'catalog', 'settings', 'questionnaire', 'editor', 'rehearsal', 'share', 'live', 'history', 'system']
...
const systemNav = { id: 'system' as const, label: 'Система', icon: ShieldCheck }
...
export function OrganizerPage() {
  ...
  const [tab, setTab] = useRememberedNavigationState<Tab>('admin-tab', 'overview', rememberedTabs)
  ...
  const checkAccount = async () => {
    try { const current = await api.me(); setAccount(current); setAuthenticated(true) }
    catch { setAccount(null); setAuthenticated(false) }
  }
  useEffect(() => { void checkAccount().finally(() => setCheckingAuth(false)) }, [])
  useEffect(() => { if (authenticated) void load(); else if (!checkingAuth) setLoading(false) }, [authenticated, checkingAuth])
  useEffect(() => { if (storeSnapshot) setSession(storeSnapshot) }, [storeSnapshot])
  useEffect(() => { if (account && account.role !== 'superadmin' && tab === 'system') setTab('overview') }, [account, tab])

  if (checkingAuth) return <div className="center-screen"><LoaderCircle className="spin" size={32} /><p>Проверяем аккаунт…</p></div>
  if (!authenticated) return <LoginPanel onDone={() => { setCheckingAuth(true); void checkAccount().finally(() => setCheckingAuth(false)) }} />
  if (loading) return <div className="center-screen"><LoaderCircle className="spin" size={32} /><p>Готовим панель…</p></div>
  if (creatingEvent || !event) return <CreateEventPanel onCreated={async () => { setCreatingEvent(false); await refreshBranding(); await load() }} onLogout={() => { void api.logout(); setAuthenticated(false) }} onCancel={event ? () => setCreatingEvent(false) : undefined} />

  const logout = () => { void api.logout(); setTab('overview'); setAccount(null); setAuthenticated(false) }
  const eventNav = nav.filter(item => event.event_format === 'celebration' || item.id !== 'questionnaire')
  const mobileNav = [...eventNav.filter(item => ['overview', 'quizzes', 'editor', 'live', 'history'].includes(item.id)), ...(account?.role === 'superadmin' ? [systemNav] : [])]
  ...
  return <div className="admin-shell" style={{ '--accent': event.theme.accent } as React.CSSProperties}>
    <aside className="admin-sidebar">
      ...
      <div className="sidebar-bottom">{account?.role === 'superadmin' && <button className={`system-entry ${tab === 'system' ? 'active' : ''}`} onClick={() => setTab('system')}><ShieldCheck size={19} /> <span>Система</span></button>}<div className="mode-block"><span className="status-dot" /><div><b>{session?.session.deployment_mode === 'cloud' ? 'Облачный режим' : 'Локальный режим'}</b><small>Сервер доступен</small></div></div><Link to="/account"><Settings2 size={19} /> Аккаунт</Link><button onClick={logout}><LogOut size={19} /> Выйти</button></div>
    </aside>
    <div className="admin-main">
      <header className="admin-topbar"><button className="mobile-logo" onClick={() => setTab('overview')}><Logo compact /></button>{tab === 'system' ? <div className="current-quiz-switch is-static"><h1><ShieldCheck size={16} /> Управление системой</h1></div> : <button className="current-quiz-switch" onClick={() => setTab('quizzes')}><span className="crumb">Выбранный квиз · {events.filter(item => item.status !== 'archived').length} в библиотеке</span><h1>{event.title} <ChevronRight size={15} /></h1></button>}<div className="top-actions">{tab !== 'system' && <>{event.active_session_code && session && <Badge tone="success"><Radio size={13} /> Комната {session.session.join_code}</Badge>}<Button variant="secondary" onClick={() => void load()}><Activity size={17} /> Обновить</Button></>}</div></header>
      ...
      <main className="admin-content">
        ...
        {tab === 'system' && account?.role === 'superadmin' && <SystemAdminPanel />}
      </main>
...
function LoginPanel({ onDone }: { onDone: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => { e.preventDefault(); setLoading(true); setError(''); try { await api.login(phone, password); onDone() } catch (err) { setError(err instanceof Error ? err.message : 'Не удалось войти') } finally { setLoading(false) } }
  return <main className="login-page"><Link to="/" className="back-link"><ArrowLeft size={17} /> На главную</Link><Card className="login-card"><Logo /><div className="login-intro"><Badge tone="accent">Панель организатора</Badge><h1>Соберём квиз?</h1><p>Войдите по номеру телефона или создайте личный аккаунт.</p></div><form onSubmit={submit}><Field label="Номер телефона"><input type="tel" value={phone} onChange={e => setPhone(e.target.value)} autoComplete="tel" placeholder="+77001234567" /></Field><Field label="Пароль"><input type="password" value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" /></Field>{error && <p className="form-error">{error}</p>}<Button type="submit" disabled={loading}>{loading ? <LoaderCircle className="spin" size={19} /> : <Sparkles size={19} />} Войти</Button><Link className="button button-secondary" to="/account?register=1">Создать аккаунт</Link></form><small className="demo-hint">Локальный суперадмин: +77000000000 / celebrate</small></Card></main>
}

function ScreenLaunch({ session, compact = false }: { session: Snapshot; compact?: boolean }) {
  const [url, setUrl] = useState(session.screen_url || '')
  const [busy, setBusy] = useState(false)
  useEffect(() => { if (session.screen_url) setUrl(session.screen_url) }, [session.screen_url])
  if (url) return <a className={compact ? undefined : 'button button-secondary'} href={url} target="_blank"><Monitor size={18} /> Телевизор <ExternalLink size={15} /></a>
  return <Button variant="secondary" disabled={busy || session.session.status === 'finished'} onClick={async () => { setBusy(true); try { setUrl((await api.regenerateScreenAccess(session.session.join_code)).screen_url) } finally { setBusy(false) } }}><Monitor size={18} /> {busy ? 'Создаём…' : 'TV-ссылка'}</Button>
}

function CreateEventPanel({ onCreated, onLogout, onCancel }: { onCreated: () => void; onLogout: () => void; onCancel?: () => void }) {
  const [form, setForm] = useState({
    title: '', event_format: 'celebration' as EventData['event_format'], content_mode: 'quiz' as EventData['content_mode'], topic: '', hero_name: '', event_date: new Date().toISOString().slice(0, 10),
...
function Overview({ event, session, onOpen, onTab, onChanged }: { event: EventData; session: Snapshot | null; onOpen: () => void; onTab: (tab: Tab) => void; onChanged: () => void }) {
  ...
  const [form, setForm] = useState({ title: event.title, event_format: event.event_format, content_mode: event.content_mode, topic: event.topic, hero_name: event.hero_name, event_date: event.event_date, game_mode: event.game_mode, host_mode: event.host_mode, auto_advance_seconds: event.auto_advance_seconds, tv_display_mode: event.tv_display_mode, tv_chart_style: event.tv_chart_style, allow_late_join: event.allow_late_join, hero_photo_url: event.hero_photo_url, theme: event.theme })
...
export function PackCatalogPanel({ onInstalled }: { onInstalled: () => Promise<void> | void }) {
  ...
  const [contentMode, setContentMode] = useState<ContentMode>('quiz')
  ...
  const generatePrompt = async () => {
    ...
    try {
      const result = await api.quizPackPrompt({ topic: topic.trim(), question_count: questionCount, difficulty, content_mode: contentMode })
  ...
  const saveTemplate = async () => {
    setBuilderBusy(true); setBuilderError('')
    try {
      const cleanJson = cleanQuizPackJsonInput(jsonText)
  ...
  return <div className="content-stack quiz-library-panel">
    ...
    {builderOpen && <Card className="template-builder"><div className="template-builder-head"><div><Badge tone={editingSlug ? 'warning' : 'accent'}>{editingSlug ? 'Редактирование' : 'GPT → JSON → каталог'}</Badge><h3>{editingSlug ? 'Измените JSON шаблона' : 'Создание нового шаблона'}</h3><p>{editingSlug ? 'После сохранения карточка и будущие квизы будут использовать обновлённую версию.' : 'Мы подготовим точный промпт. GPT проведёт исследование, а платформа проверит и сохранит результат.'}</p></div><button className="icon-button" aria-label="Закрыть конструктор" onClick={() => setBuilderOpen(false)}>×</button></div>
      ...
      {builderStep === 'setup' && <div className="template-setup"><div className="form-grid"><Field label="Тип"><select value={contentMode} onChange={event => setContentMode(event.target.value as ContentMode)}><option value="quiz">Квиз — рейтинг</option><option value="test">Тест — личный результат</option><option value="survey">Опрос — без оценок</option></select></Field><Field label="Тема"><input value={topic} onChange={event => setTopic(event.target.value)} placeholder={contentMode === 'survey' ? 'Например: Удовлетворённость мероприятием' : 'Например: Страны мира'} /></Field><Field label="Количество вопросов"><input type="number" min="3" max="50" value={questionCount} onChange={event => setQuestionCount(Math.max(3, Math.min(50, Number(event.target.value))))} /></Field><Field label="Сложность"><select value={difficulty} disabled={contentMode === 'survey'} onChange={event => setDifficulty(event.target.value)}><option>Лёгкая</option><option>Средняя</option><option>Сложная</option></select></Field></div><div className="template-research-note"><Sparkles /><span><b>{contentMode === 'survey' ? 'GPT подготовит нейтральные вопросы' : 'Промпт потребует исследование'}</b><small>{contentMode === 'survey' ? 'В опросе не будет правильных ответов: GPT предложит варианты мнений и свободные поля.' : 'GPT должен проверить каждый факт, приложить прямые ссылки и вернуть только строгий JSON — без Python и Markdown.'}</small></span></div><div className="template-setup-actions"><Button onClick={() => void generatePrompt()} disabled={builderBusy || !topic.trim()}>{builderBusy ? <LoaderCircle className="spin" /> : <Sparkles />} Создать промпт</Button>{prompt && <Button variant="secondary" onClick={() => setBuilderStep('prompt')}><History /> Открыть последний промпт</Button>}</div></div>}
      ...
      {builderStep === 'json' && <div className="template-json-step"><div className="template-step-title"><div><span className="overline">{editingSlug ? 'JSON шаблона' : 'Шаг 3'}</span><h4>{editingSlug ? 'Проверьте и сохраните изменения' : 'Вставьте ответ GPT'}</h4><p>Вставьте ответ целиком. Платформа уберёт служебный Markdown и исправит типичные ошибки в ссылках, затем проверит режим, вопросы и источники.</p></div>{!editingSlug && prompt && <Button variant="secondary" onClick={() => setBuilderStep('prompt')}><Copy /> Вернуться к промпту</Button>}</div><textarea className="json-import-area" value={jsonText} onChange={event => changeJson(event.target.value)} rows={20} spellCheck={false} placeholder={'{\n  "schema_version": 1,\n  "content_mode": "quiz",\n  "slug": "countries-world",\n  ...\n}'} />{builderError && <p className="form-error json-validation-error"><b>JSON нужно поправить</b>{'\n'}{builderError}</p>}<div className="template-builder-actions"><small>Существующие материалы не изменятся — сохраняется только шаблон каталога.</small><Button onClick={() => void saveTemplate()} disabled={builderBusy || !jsonText.trim()}>{builderBusy ? <LoaderCircle className="spin" /> : <Save />} {editingSlug ? 'Сохранить изменения' : 'Проверить и сохранить'}</Button></div></div>}
...
function QuestionForm({ event, question, onSaved, onSpeechChanged, onDelete }: { event: EventData; question: Question | null; onSaved: (saved: Question) => void; onSpeechChanged: () => void | Promise<void>; onDelete?: () => void }) {
  ...
  const save = async () => { setSaving(true); setError(''); try { const scoringPayload = { ...form, accepted_answers: typeof form.accepted_answers === 'string' ? form.accepted_answers.split(',').map((x: string) => x.trim()).filter(Boolean) : form.accepted_answers, correct_answer: ['number', 'closest'].includes(form.type) ? Number(form.correct_answer) : form.correct_answer }; const payload = event.content_mode === 'survey' ? { ...scoringPayload, correct_answer: null, accepted_answers: [], numeric_tolerance: 0, options: scoringPayload.options.map((option: any) => ({ ...option, is_correct: false })) } : scoringPayload; const saved = question ? await api.updateQuestion(question.id, payload) : await api.createQuestion(event.id, payload); localStorage.removeItem(draftKey); setHasDraft(false); onSaved(saved) } catch (err) { setError(err instanceof Error ? err.message : 'Не удалось сохранить вопрос') } finally { setSaving(false) } }
...
function SharePanel({ event, session, onOpen, onChanged }: { event: EventData; session: Snapshot | null; onOpen: () => void; onChanged: () => void | Promise<void> }) {
  ...
  const [screenUrl, setScreenUrl] = useState(session?.screen_url || '')
  useEffect(() => {
  ...
  useEffect(() => { if (session?.screen_url) setScreenUrl(session.screen_url) }, [session?.screen_url])
  const createScreenLink = async () => { if (!code) return; setBusy(true); try { const result = await api.regenerateScreenAccess(code); setScreenUrl(result.screen_url) } catch (err) { setError(err instanceof Error ? err.message : 'Не удалось создать TV-ссылку') } finally { setBusy(false) } }
  ...
  return <div className="content-stack tv-share-page">
    <section className="page-heading"><div><Badge tone="accent"><Share2 size={14} /> Share / ТВ</Badge><h2>Экран для гостей</h2><p>Телевизор подключается по отдельной отзываемой ссылке без входа в аккаунт.</p></div>{code ? screenUrl ? <a className="button" href={screenUrl} target="_blank"><Monitor size={18} /> Открыть телевизор</a> : <Button onClick={() => void createScreenLink()}><Monitor size={18} /> Создать TV-ссылку</Button> : <Button onClick={onOpen}><Play size={18} /> Открыть комнату</Button>}</section>
...
function ResultsPanel({ event, session, onReplay }: { event: EventData; session: Snapshot | null; onReplay: () => void }) {
  ...
  return <div className="content-stack"><section className="page-heading"><div><Badge tone={selectedSession?.status === 'finished' ? 'success' : 'neutral'}>{selectedSession?.status === 'finished' ? 'Материал завершён' : 'Предварительные данные'}</Badge><h2>История запусков: {event.title}</h2><p>Каждый повторный запуск хранится отдельно. Выберите комнату, чтобы посмотреть результаты и ответы.</p></div><Badge>{event.sessions.length} запусков</Badge></section><Card className="session-history-picker">{event.sessions.map((item, index) => <button key={item.id} className={selectedCode === item.join_code ? 'active' : ''} onClick={() => setSelectedCode(item.join_code)}><span>{index + 1}</span><div><b>Комната {item.join_code}</b><small>{item.participant_count} участников · {item.status === 'finished' ? 'завершена' : 'в процессе'}</small></div>{selectedCode === item.join_code && <Check />}</button>)}</Card>{ranking.length ? <Card className="results-table"><div className="table-row table-head"><span>Место</span><span>Игрок</span><span>Верно</span><span>Время</span></div>{ranking.map((row: any) => <div className="table-row" key={row.id}><strong>{row.rank}</strong><span className="player-cell"><i>{row.avatar}</i><b>{row.name}</b></span><span>{row.correct_count}</span><span>{formatTime(row.correct_time_ms)}</span></div>)}</Card> : event.content_mode === 'survey' ? <Empty icon="✦" title="Опрос без рейтинга" text="Ни один ответ не оценивается — ниже доступен полный журнал мнений." /> : <Empty icon="✦" title="Ответов ещё нет" text="Результаты появятся после раскрытия вопросов." />}{results?.submissions?.length > 0 && <Card><div className="section-title"><div><span className="overline">Журнал</span><h3>Все ответы</h3></div></div><div className="submission-list">{results.submissions.map((row: any) => <div key={row.id}><span className={row.is_correct == null ? '' : row.is_correct ? 'answer-ok' : 'answer-no'}>{row.is_correct == null ? '•' : row.is_correct ? '✓' : '×'}</span><div><b>{row.name}</b><small>{row.question}</small></div><code>{String(row.answer ?? 'Пропуск')}</code><span>{formatTime(row.elapsed_ms)}</span></div>)}</div></Card>}</div>
}
