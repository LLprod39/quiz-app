...
from .models import (
    ...
    QuestionnaireItem, QuestionnaireResponse, QuestionSpeechVersion, QuizPackTemplate, Round, ScreenAccess, ScreenDevice,
    Submission, Team, uid,
)
...
from .security import (
    create_speech_upload_ticket, new_device_token, optional_account, parse_user_agent,
    request_ip, require_admin, token_hash, verify_speech_upload_ticket,
)
...
def speech_question(db: Session, question_id: str, account: Account | None = None) -> Question:
    question = db.scalar(
    ...
    if not question:
        raise HTTPException(404, "Вопрос не найден")
    if account and question.round.event.owner_id != account.id:
        raise HTTPException(404, "Вопрос не найден")
    return question
...
def speech_mvp_config(_account: Account = Depends(require_admin)):
    return {
...
def update_event_speech_settings(
    ...
    db.add(AuditLog(actor_account_id=account.id, target_account_id=account.id, action="event.speech_settings.updated", before=before, after=event.speech_settings))
    db.commit()
    return event.speech_settings


@router.get("/questions/{question_id}/speech")
def question_speech(question_id: str, account: Account = Depends(require_admin), db: Session = Depends(get_db)):
    question = speech_question(db, question_id, account)
    return {
...
def activate_question_speech(
    ...
    db.add(AuditLog(actor_account_id=account.id, target_account_id=account.id, action="question.speech.activated", after={"question_id": question.id, "version_id": target.id}))
    db.commit()
...
def restore_speech_version(db: Session, question: Question, previous: QuestionSpeechVersion, account: Account) -> dict:
    removed: list[str] = []
    for version in question.speech_versions:
    ...
    db.add(AuditLog(actor_account_id=account.id, target_account_id=account.id, action="question.speech.restored", after={"question_id": question.id, "version_id": previous.id}))
    db.commit()
    for file_url in removed:
        remove_speech_file(file_url)
    return serialize_question_speech(speech_question(db, question.id))
...
def restore_question_speech_version(
    ...
    return restore_speech_version(db, question, previous, account)


@router.post("/questions/{question_id}/speech/restore")
def restore_question_speech(question_id: str, account: Account = Depends(require_admin), db: Session = Depends(get_db)):
    question = speech_question(db, question_id, account)
...
def delete_question_speech_version(
    ...
    version.status = "discarded"
    db.add(AuditLog(actor_account_id=account.id, target_account_id=account.id, action="question.speech.deleted", before={"question_id": question.id, "version_id": version.id, "status": deleted_status}))
    db.commit()
    remove_speech_file(file_url)
