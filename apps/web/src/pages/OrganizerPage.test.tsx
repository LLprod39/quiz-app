...
function snapshot(status: Snapshot['session']['status']): Snapshot {
  return {
    ...
    event: { id: event.id, title: event.title, event_format: event.event_format, topic: event.topic, hero_name: '', game_mode: event.game_mode, theme: DEFAULT_BRANDING },
...
describe('LivePanel final flow', () => {
  it('shows a clear results action when the game is already finished', () => {
    ...
    render(<LivePanel event={event} session={snapshot('finished')} onOpen={vi.fn()} onResults={onResults} />)
  ...
  it('opens results automatically after showing the final', async () => {
    ...
    const onResults = vi.fn()
    render(<LivePanel event={event} session={snapshot('reveal')} onOpen={vi.fn()} onResults={onResults} />)
