...
export type ContentMode = 'quiz' | 'test' | 'survey'
...
export interface EventData {
  id: string; title: string; event_format: 'celebration' | 'battle'; content_mode: ContentMode; topic: string
...
export interface Ranking { id: string; name: string; avatar: string; color?: string; correct_count: number; correct_time_ms: number; rank: number | null; question_count?: number; score_percent?: number; answer?: unknown; elapsed_ms?: number; is_correct?: boolean | null }
...
export interface Snapshot {
  ...
  event: { id: string; title: string; event_format: 'celebration' | 'battle'; content_mode: ContentMode; topic: string; hero_name: string; hero_photo_url?: string; game_mode: string; host_mode: 'auto' | 'manual'; auto_advance_seconds: number; tv_display_mode: 'classic' | 'insights'; tv_chart_style: 'both' | 'pie' | 'bar'; theme: ThemeConfig }
