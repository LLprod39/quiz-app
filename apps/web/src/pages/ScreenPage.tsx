...
export function ScreenPage() {
  const screenToken = useParams().code!; const snapshot = useGameStore(s => s.snapshot); const connection = useGameStore(s => s.connection); const latency = useGameStore(s => s.latency); const [showTable, setShowTable] = useState(false)
  const speech = useQuestionSpeech({ sessionId: snapshot?.session.id, sessionCode: screenToken, status: snapshot?.session.status, question: snapshot?.question })
  useEffect(() => { useGameStore.getState().connectScreen(screenToken); const key = (e: KeyboardEvent) => { if (e.key.toLowerCase() === 'f') void toggleFull() }; window.addEventListener('keydown', key); return () => { window.removeEventListener('keydown', key); useGameStore.getState().disconnect() } }, [screenToken])
