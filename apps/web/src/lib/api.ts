...
export const api = {
  ...
  quizPackPrompt: (data: { topic: string; question_count: number; difficulty: string; content_mode: 'quiz' | 'test' | 'survey' }) => request<{ prompt: string; topic: string; question_count: number; content_mode: 'quiz' | 'test' | 'survey' }>('/quiz-packs/gpt-prompt', { method: 'POST', body: JSON.stringify(data) }, true),
