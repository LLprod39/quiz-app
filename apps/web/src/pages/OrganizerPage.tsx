...
export function OrganizerPage() {
  ...
  return <div className="admin-shell" style={{ '--accent': event.theme.accent } as React.CSSProperties}>
    <aside className="admin-sidebar">
      <Logo />
      <nav>{nav.map(item => <button key={item.id} className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)}><item.icon size={20} /><span>{item.label}</span>{item.id === 'live' && session && <i className="live-dot" />}</button>)}</nav>
    ...
    <div className="admin-main">
      ...
      <main className="admin-content">
        ...
        {tab === 'questionnaire' && <QuestionnairePanel event={event} onChanged={load} />}
      ...
      <nav className="mobile-tabs">{nav.slice(0, 5).map(item => <button key={item.id} className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)}><item.icon size={20} /><span>{item.label}</span></button>)}</nav>
    </div>
  </div>
}
...
function CreateEventPanel({ onCreated, onLogout }: { onCreated: () => void; onLogout: () => void }) {
  ...
  return <main className="login-page create-event-page">
    ...
    <Card className="login-card create-event-card"><Logo /><div className="login-intro"><Badge tone="accent">Новое мероприятие</Badge><h1>Кого сегодня празднуем?</h1><p>Создайте основу — анкета и первый раунд появятся автоматически.</p></div>
      <form onSubmit={submit} className="form-grid"><Field label="Название"><input required minLength={2} value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Вечер в честь…" /></Field><Field label="Имя героя"><input required value={form.hero_name} onChange={e => setForm({ ...form, hero_name: e.target.value })} placeholder="Имя" /></Field><Field label="Дата"><input type="date" value={form.event_date} onChange={e => setForm({ ...form, event_date: e.target.value })} /></Field><Field label="Режим"><select value={form.game_mode} onChange={e => setForm({ ...form, game_mode: e.target.value })}><option value="individual">Личный</option><option value="team">Командный</option></select></Field><Field label="Акцентный цвет"><input type="color" value={form.theme.accent} onChange={e => setForm({ ...form, theme: { ...form.theme, accent: e.target.value } })} /></Field><label className="check-row"><input type="checkbox" checked={form.allow_late_join} onChange={e => setForm({ ...form, allow_late_join: e.target.checked })} /><span><b>Разрешить поздний вход</b><small>Опоздавшие начнут со следующего вопроса</small></span></label>{error && <p className="form-error form-grid-wide">{error}</p>}<Button type="submit" className="form-grid-wide" disabled={busy || !form.title.trim() || !form.hero_name.trim()}>{busy ? <LoaderCircle className="spin" size={19} /> : <PartyPopper size={19} />} Создать мероприятие</Button></form>
    </Card>
  </main>
}
...
function Overview({ event, session, onOpen, onTab, onChanged }: { event: EventData; session: Snapshot | null; onOpen: () => void; onTab: (tab: Tab) => void; onChanged: () => void }) {
  ...
  const [form, setForm] = useState({ title: event.title, hero_name: event.hero_name, event_date: event.event_date, game_mode: event.game_mode, allow_late_join: event.allow_late_join, hero_photo_url: event.hero_photo_url, theme: event.theme })
  ...
  return <div className="content-stack">
    ...
    <div className="stats-grid"><Card><span className="metric-icon coral"><CircleHelp /></span><div><strong>{event.question_count}<small>/ 15</small></strong><span>вопросов готово</span></div><button onClick={() => onTab('editor')}>Редактировать <ChevronRight /></button></Card><Card><span className="metric-icon purple"><ClipboardList /></span><div><strong>{event.questionnaire?.status === 'completed' ? 'Готова' : 'Ждём'}</strong><span>анкета героя</span></div><button onClick={() => onTab('questionnaire')}>Открыть <ChevronRight /></button></Card><Card><span className="metric-icon mint"><Users /></span><div><strong>{session?.participants.length || 0}</strong><span>гостей в комнате</span></div><button onClick={() => onTab('live')}>Посмотреть <ChevronRight /></button></Card></div>
    <div className="overview-grid"><Card className="event-card"><div className="card-title"><div><span className="overline">Карточка события</span><h3>{event.title}</h3></div><div className="event-card-actions"><button className="icon-button" title="Редактировать" onClick={() => setEditing(!editing)}><Pencil size={18} /></button><button className="icon-button danger-icon" title="Архивировать" onClick={() => void archive()} disabled={saving}><Archive size={18} /></button></div></div>{editing ? <div className="form-grid"><Field label="Название"><input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></Field><Field label="Имя героя"><input value={form.hero_name} onChange={e => setForm({ ...form, hero_name: e.target.value })} /></Field><Field label="Дата"><input type="date" value={form.event_date} onChange={e => setForm({ ...form, event_date: e.target.value })} /></Field><Field label="Режим"><select value={form.game_mode} onChange={e => setForm({ ...form, game_mode: e.target.value })}><option value="individual">Личный</option><option value="team">Командный</option></select></Field><Field label="Акцентный цвет"><input type="color" value={form.theme.accent} onChange={e => setForm({ ...form, theme: { ...form.theme, accent: e.target.value } })} /></Field><label className="check-row"><input type="checkbox" checked={form.allow_late_join} onChange={e => setForm({ ...form, allow_late_join: e.target.checked })} /><span><b>Разрешить поздний вход</b><small>Гость начнёт со следующего вопроса</small></span></label><div className="form-actions"><Button variant="ghost" onClick={() => setEditing(false)}>Отмена</Button><Button onClick={() => void save()} disabled={saving}><Save size={17} /> Сохранить</Button></div></div> : <><div className="event-hero-preview"><div className="event-initial">{event.hero_name.slice(0, 1)}</div><div><span>Герой праздника</span><b>{event.hero_name}</b><small>{event.event_date ? new Date(event.event_date).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Дата не указана'}</small></div></div><div className="event-meta"><span><Gamepad2 /> {event.game_mode === 'team' ? 'Командная игра' : 'Личная игра'}</span><span><Wifi /> Поздний вход {event.allow_late_join ? 'включён' : 'выключен'}</span></div></>}</Card>
      <Card className="readiness-card"><div className="card-title"><div><span className="overline">Готовность</span><h3>До старта — три шага</h3></div><b>{progress}%</b></div><div className="progress-line"><i style={{ width: `${progress}%` }} /></div><button className="check-step done" onClick={() => onTab('questionnaire')}><Check /><span><b>Оформить мероприятие</b><small>Имя, дата и тема готовы</small></span><ChevronRight /></button><button className={`check-step ${event.question_count >= 3 ? 'done' : ''}`} onClick={() => onTab('editor')}><Check /><span><b>Подготовить вопросы</b><small>{event.question_count} из 15 добавлено</small></span><ChevronRight /></button><button className="check-step" onClick={() => onTab('rehearsal')}><Check /><span><b>Провести репетицию</b><small>Экран, звук и телефоны</small></span><ChevronRight /></button></Card></div>
  </div>
}
