...
const event = {
  id: 'event-1', title: 'Тестовый квиз', event_format: 'battle', content_mode: 'quiz', topic: 'Кино', hero_name: '', event_date: '', status: 'ready', is_selected: true,
...
function snapshot(status: Snapshot['session']['status']): Snapshot {
  return {
    ...
    event: { id: event.id, title: event.title, event_format: event.event_format, content_mode: event.content_mode, topic: event.topic, hero_name: '', game_mode: event.game_mode, host_mode: 'auto', auto_advance_seconds: 5, tv_display_mode: 'classic', tv_chart_style: 'both', theme: DEFAULT_BRANDING },
...
describe('PackCatalogPanel GPT builder', () => {
  it('creates a topic prompt and saves pasted JSON as a permanent template', async () => {
    const customPack = {
      ...
      description: 'Готовый тематический квиз о странах, столицах и географии.', estimated_minutes: 30, difficulty: 'Средняя', content_mode: 'quiz' as const, game_mode: 'team' as const,
    ...
    vi.spyOn(api, 'quizPackPrompt').mockResolvedValue({ prompt: 'Найди надёжные источники и верни JSON', topic: 'Страны мира', question_count: 20, content_mode: 'quiz' })
    ...
    fireEvent.change(screen.getByLabelText('Тема'), { target: { value: 'Страны мира' } })
  ...
  it('passes the selected survey mode into GPT prompt generation', async () => {
    vi.spyOn(api, 'quizPacks').mockResolvedValue([])
    const promptRequest = vi.spyOn(api, 'quizPackPrompt').mockResolvedValue({ prompt: 'Верни JSON опроса', topic: 'Обратная связь', question_count: 20, content_mode: 'survey' })

    render(<MemoryRouter><PackCatalogPanel onInstalled={vi.fn()} /></MemoryRouter>)
    await waitFor(() => expect(api.quizPacks).toHaveBeenCalled())
    fireEvent.click(screen.getByRole('button', { name: /создать шаблон с gpt/i }))
    fireEvent.change(screen.getByLabelText('Тип'), { target: { value: 'survey' } })
    fireEvent.change(screen.getByLabelText('Тема'), { target: { value: 'Обратная связь' } })
    fireEvent.click(screen.getByRole('button', { name: /^создать промпт$/i }))

    await waitFor(() => expect(promptRequest).toHaveBeenCalledWith({ topic: 'Обратная связь', question_count: 20, difficulty: 'Средняя', content_mode: 'survey' }))
    expect(await screen.findByDisplayValue('Верни JSON опроса')).toBeInTheDocument()
  })
})
